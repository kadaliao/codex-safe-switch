...
在官方 ChatGPT 登录、第三方 relay、多组 API key 之间一键切换 [Codex CLI](https://github.com/openai/codex) 的 provider 配置。CLI + 可选 Alfred workflow。
...
codex-safe-switch official     # 切回官方 OpenAI ChatGPT 登录(别名:openai)
...
**`auth.json` 只在必要时复制。** 只有 provider 明确需要 OpenAI/ChatGPT auth,或没声明 `requires_openai_auth = false` 的旧式 API-key 配置才会保存/写回 `auth.json`。使用 `env_key` 的 relay 切换时不会覆盖你的官方 ChatGPT 登录缓存——Codex 远程连接可以继续用同一个 ChatGPT 账号。
...
**官方 OpenAI 一键回退。** `codex-safe-switch official` 切回官方 ChatGPT 登录,工具维护隐藏快照 `~/.codex/profiles/.official/`,第一次从官方切走时自动刷新。
...
├── .official/
│   ├── auth.json                 # 切换时复制到 ~/.codex/auth.json
│   └── provider.toml             # 官方登录的 provider 片段
└── myrelay/
    ├── auth.json                 # 可选;只有该 profile 自带 key/token 时需要
...
2. 如果 key 来自环境变量,推荐用 `requires_openai_auth = false` + `env_key = "..."`,这样不需要 `auth.json`,切换时保留官方 ChatGPT 登录缓存。
